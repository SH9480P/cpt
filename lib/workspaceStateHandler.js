...
function updateCodeChange(context, addNum, deleteNum) {
    if (addNum !== 0 || deleteNum !== 0) {
        ...
        if (changeObject.YMDHm !== currentYMDHm) {
            updateCompleteCodeChange(context, changeObject.addTotal, changeObject.deleteTotal, changeObject.YMDHm)
...
function updateCodingDuration(context) {
    ...
    if (durationObject.YMDHm !== currentYMDHm) {
        ...
        if (dayjs(nextYMDHm).diff(durationObject.lastTriggeredTime.add(30, 's')) > 0) {
            ...
            updateCompleteCodingDuration(context, durationObject.longTotal + dayjs(nextYMDHm).diff(durationObject.lastTriggeredTime), durationObject.YMDHm)
            updateCompleteCodingDuration(context, dayjs.min(durationObject.lastTriggeredTime.add(30, 's'), now).diff(nextYMDHm), nextYMDHm)
        }

        durationObject.YMDHm = currentYMDHm
        durationObject.longTotal = 0
        if (nextYMDHm === currentYMDHm && durationObject.lastTriggeredTime.add(30, 's').isAfter(dayjs(currentYMDHm))) {
            durationObject.longTotal = dayjs.min(now, durationObject.lastTriggeredTime.add(30, 's')).diff(dayjs(currentYMDHm))
        }
